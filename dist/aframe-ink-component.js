    ...
    class CallStack {
        constructor() {
            ...
            if (arguments[0] instanceof Container || arguments[0] === null) {
                let rootContentContainer = arguments[0];
                this._threads = [];
...
AFRAME.registerComponent('ink-state', {
  ...
  init: function () {
    ...
    el.addEventListener(INK_LOADED_EVENT, (evt) => {
...
//      for (key in this.system.state) {
      for (key in stateSystem.state) {
        debugger;
        for (inkKey in inkStory.variablesState._globalVariables.keys()) {
        //inkStory.variablesState._globalVariables.keys(inkKey => {
          if (inkKey === key) {
  ...
  stateUpdated: function (evt) {
    ...
    //  for (inkKey in this.inkStory.variablesState._globalVariables) {
        this.inkStory.variablesState._globalVariables.forEach(inkKey => {
